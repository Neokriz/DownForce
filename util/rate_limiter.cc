...
void GenericRateLimiter::Request(int64_t bytes, const Env::IOPriority pri,
  ...
  if (available_bytes_ > 0) {
    ...
    if (pri == Env::IO_LOW) {
      int64_t max_io_low = (refill_bytes_per_period_.load(std::memory_order_relaxed) / 3) -
  ...
  for (int i = Env::IO_LOW; i < Env::IO_TOTAL; ++i) {
    ...
    while (!queue->empty()) {
      ...
      if (current_pri == Env::IO_LOW) {
        int64_t max_io_low = (refill_bytes_per_period / 3) - io_low_bytes_this_period_;
        max_io_low = std::max<int64_t>(0, max_io_low);
      ...
      if (max_grant < next_req->request_bytes) {
        ...
        // - For IO_LOW: capped at refill_bytes_per_period/2 per period
      ...
      int64_t granted = next_req->request_bytes;
